...
- **Global scope**: `/Users/familyaccount/git/FinanceApp/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/familyaccount/git/FinanceApp/.claude/notes/`)
...
2. Glob `/Users/familyaccount/git/FinanceApp/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/familyaccount/git/FinanceApp/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/familyaccount/git/FinanceApp/.claude/notes/` — works in any directory
